...
from .cuda2hip_mappings import CUDA_TO_HIP_MAPPINGS, MATH_TRANSPILATIONS
...
def matched_files_iter(
    ...
    for abs_dirpath, dirs, filenames in os.walk(root_path, topdown=True):
        ...
        for filename in filenames:
            ...
            _to_unix_path(os.path.join(rel_dirpath, filename))
                ...
                if header_filepath not in HIPIFY_FINAL_RESULT:
                    ...
                    if header_result.current_state == CurrentState.INITIALIZED:
                        ...
                        header_fout_path = os.path.abspath(
                            os.path.join(
                                output_directory, get_hip_file_path(header_rel_path, is_pytorch_extension)
                            )
                        )
                        ...
                        return templ.format(
                            os.path.relpath(
                                header_fout_path if header_fout_path is not None else header_filepath,
                                header_dir,
                            )
                        )
