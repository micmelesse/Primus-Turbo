...
class Buffer:
    ...
    def __init__(
        ...
        if self.runtime.is_low_latency_optimize():
            # Synchronize pxn IPC handles
            pxn_ipc_handles = [
                None,
            ] * self.group_size
            local_pxn_ipc_handle = self.runtime.get_local_pxn_ipc_handle()
            dist.all_gather_object(pxn_ipc_handles, local_pxn_ipc_handle, group)
            self.runtime.sync_pxn_handles(device_ids, pxn_ipc_handles)
    ...
    def combine(
        ...
        if self.runtime.get_num_rdma_ranks() > 1:
            return self.internode_combine(
                x, handle, topk_weights, config, previous_event, async_finish, allocate_on_comm_stream
            )
