...
#define DECL_GET_CK_GG_INSTANCE_EXTERN(A, B, C, AL, BL, CL)         \
    extern template std::unique_ptr<CKGroupedGemmRunnerInterFace>   \
    get_ck_grouped_gemm_instance<A, B, C, float, AL, BL, CL>(       \
        const ck_tile::index_t, const ck_tile::index_t,             \
        const ck_tile::index_t, const ck_tile::index_t);

#define APPLY_GET_CK_GG_INSTANCE_ALL_LAYOUT(MACRO, A, B, C) \
    MACRO(A, B, C, RowMajor, ColMajor, RowMajor)            \
    MACRO(A, B, C, RowMajor, RowMajor, RowMajor)            \
    MACRO(A, B, C, ColMajor, RowMajor, RowMajor)
