...
at::Tensor fp8_quantize_row_col(at::Tensor &input, at::Tensor &scale,
    ...
    if (dims == 2) {
        ...
        if (input.dtype() == at::kBFloat16) {
            ...
            using OutType   = ck_tile::fp8_t; // Default to fp8_t

            if (dest_dtype == at::kFloat8_e4m3fn || dest_dtype == at::kFloat8_e4m3fnuz) {
                quant_2d<InType, ScaleType, OutType>(
                ...
                using OutType2 = ck_tile::bf8_t; // For E5M2 formats
                quant_2d<InType, ScaleType, OutType2>(
                    reinterpret_cast<const InType *>(input.data_ptr()),
                    reinterpret_cast<const ScaleType *>(scale.data_ptr()),
                    reinterpret_cast<OutType2 *>(output.data_ptr()), m, k, !is_row_major, stream);
            }
        ...
        if (input.dtype() == at::kBFloat16) {
            ...
            if (dest_dtype == at::kFloat8_e4m3fn || dest_dtype == at::kFloat8_e4m3fnuz) {
                quant_3d<InType, ScaleType, OutType>(
                    reinterpret_cast<const InType *>(input.data_ptr()),
                    reinterpret_cast<const ScaleType *>(scale.data_ptr()),
                    reinterpret_cast<OutType *>(output.data_ptr()), b, m, k, !is_row_major, stream);
            } else {
                using OutType2 = ck_tile::bf8_t; // For E5M2 formats
                quant_3d<InType, ScaleType, OutType2>(
                    reinterpret_cast<const InType *>(input.data_ptr()),
                    reinterpret_cast<const ScaleType *>(scale.data_ptr()),
                    reinterpret_cast<OutType2 *>(output.data_ptr()), b, m, k, !is_row_major,
                    stream);
            }
        } else if (input.dtype() == at::kHalf) {
            using InType    = typename TorchToCKTileType<at::kHalf>::type;
...
at::Tensor grouped_gemm_fp8_dequant(at::Tensor &input, at::Tensor &group_lens,
                                    ...
                                    at::Tensor &scale_b) {
    PRIMUS_TURBO_CHECK(scale_a.scalar_type() == at::kFloat);
    PRIMUS_TURBO_CHECK(scale_b.scalar_type() == at::kFloat);
    int64_t dims = input.ndimension();
    ...
    auto stream = at::cuda::getCurrentCUDAStream();
    if (input.dtype() == at::kBFloat16) {
        using InType        = typename TorchToCKTileType<torch::kBFloat16>::type;
        using ScaleType     = typename TorchToCKTileType<torch::kFloat>::type;
        using OutType       = typename TorchToCKTileType<torch::kBFloat16>::type;
        ck_tile::index_t bs = group_lens.numel();
        ck_tile::index_t ms = m / bs;
        dequant_grouped_gemm<InType, ScaleType, OutType>(
            reinterpret_cast<const InType *>(input.data_ptr()),
        ...
        using InType        = typename TorchToCKTileType<at::kHalf>::type;
        using ScaleType     = typename TorchToCKTileType<torch::kFloat>::type;
        using OutType       = typename TorchToCKTileType<at::kHalf>::type;
        ck_tile::index_t bs = group_lens.numel();
        ck_tile::index_t ms = m / bs;
        dequant_grouped_gemm<InType, ScaleType, OutType>(
            reinterpret_cast<const InType *>(input.data_ptr()),
...
at::Tensor grouped_gemm_fp8_dequant_variable_k(at::Tensor &input, at::Tensor &scale_a,
                                               at::Tensor &scale_b) {
    int64_t dims = input.ndimension();
    PRIMUS_TURBO_CHECK(dims == 3, "only support 3D tensors");
    int64_t    b = 0, n = 0, k = 0;
    at::Tensor output;
    ...
    if (input.dtype() == at::kBFloat16) {
        ...
        dequant_grouped_gemm_variable_k<InType, ScaleType, OutType>(
            reinterpret_cast<const InType *>(input.data_ptr()),
            reinterpret_cast<const ScaleType *>(scale_a.data_ptr()),
            reinterpret_cast<const ScaleType *>(scale_b.data_ptr()),
            reinterpret_cast<OutType *>(output.data_ptr()), b, n, k, stream);
    } else if (input.dtype() == at::kHalf) {
        using InType    = typename TorchToCKTileType<at::kHalf>::type;
        using ScaleType = typename TorchToCKTileType<torch::kFloat>::type;
        using OutType   = typename TorchToCKTileType<at::kHalf>::type;
